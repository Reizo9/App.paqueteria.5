...
(async function(){
  ...
  if(document.body.classList.contains('page-login')){
    ...
    if(loggedInUser){ location.href = 'main.html'; return; } // <-- Redirige si SÍ hay sesión
  ...
  if(document.body.classList.contains('page-main')){
    ...
    if(!user){ location.href='index.html'; return; } // <-- Redirige si NO hay sesión
    ...
    async function refreshPaquetes(){
      ...
      rows.forEach(p=>{
        ...
        // --- CORRECCIÓN 3: Añadir paquetería al historial (YA ESTABA) ---
    ...
    // --- CORRECCIÓN 2: Pop-up de entrega múltiple (con búsqueda parcial) (YA ESTABA) ---
    ...
    recibirBtn.addEventListener('click', async ()=>{
      ...
      // Obtenemos los paquetes *antes* de guardar
      const paqs = await getAll('paquetes');
      ...
      try{
        ...
        // --- INICIO: SECCIÓN DE NOTIFICACIÓN MODIFICADA ---
        if (notificarSi.checked) {
          const dom = domicilioInput.value.trim(); 
          let domInfo = null;
          if (dom) { const doms = await getAll('domicilios'); domInfo = doms.find(d => d.calle === dom); }

          // --- CÁLCULO DE PAQUETES EN CASETA ---
          // `paqs` es la lista ANTES de guardar el item actual.
          let contadorPaquetes = 0;
          if (dom) {
              contadorPaquetes = paqs.filter(pkg => 
                  pkg.domicilio === dom && 
                  pkg.estado === 'en_caseta'
              ).length;
          }
          
          // `p` es el paquete si se encontró por GUIA.
          // Si no se encontró (`!p`), es un paquete nuevo, sumamos 1.
          if (!p) {
              contadorPaquetes += 1;
          } 
          // Si se encontró (`p`), pero su estado NO ERA 'en_caseta',
          // significa que estamos actualizando un paquete que no estaba contado, sumamos 1.
          else if (p && p.estado !== 'en_caseta') {
              contadorPaquetes += 1;
          }
          // Si `p` existe y ya estaba 'en_caseta', el contador ya es correcto.
          // --- FIN CÁLCULO DE PAQUETES EN CASETA ---

          // --- INICIO NUEVO MENSAJE ---
          const nombreRes = nombre.trim() || `residente del ${dom || 'N/A'}`;
          const paqueteria = paqueteriaInput.value.trim() || 'N/A';
          
          // Datos extra
          const comentariosInfo = comentarios ? `\nComentarios: ${comentarios}` : '';
          const recibidoPorInfo = `\nRecibido por: ${user.nombre}`;

          // Plantilla
          const header = `✅*PAQUETERIA EN CASETA*✅`;
